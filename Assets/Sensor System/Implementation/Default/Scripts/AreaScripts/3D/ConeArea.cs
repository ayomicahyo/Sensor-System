...
{
    [CreateAssetMenu(menuName = "Sensor System/Default/Area/Cone Area", fileName = "Cone Area")]
    public class ConeArea : Area
    {
        ...
        {
            return new Vector3(trans.position.x, trans.position.y + high, trans.position.z);
        }
        ...
        public override void DrawArea(Transform transform)
        {
            ...
            Handles.DrawWireDisc(transform.position, transform.up, radius + 0.1f);
            ...
            for (int i = 0; i <= lines; i++)
            {
                ...
                Handles.DrawLine(TopPosititon(transform), transform.position + DirFromAngle(angle, true, transform, true));
            }
        }
        ...
        public override void Scanning(List<ITarget> Targets, Transform obj, ISensor sensor)
        {
            ...
            for (int i = 0; i <= lines; i++)
            {
                ...
                RaycastHit hit;
                bool hits = Physics.Linecast(TopPosititon(obj), obj.position + DirFromAngle(angle, true, obj, true), out hit);
